...
export class DatabaseConnection {
  ...
  async connect(config: DatabaseConfig): Promise<void> {
    ...
    try {
      ...
      console.log('Client Service MongoDB connection established successfully');
  ...
  async disconnect(): Promise<void> {
    ...
    try {
      ...
      console.log('Client Service MongoDB connection closed successfully');
    } catch (error) {
      throw new AppError(
  ...
  private setupEventListeners(): void {
    ...
    this.connection.on('connected', () => {
      console.log('Client Service MongoDB connection established');
      this._isConnected = true;
    });

    this.connection.on('error', (error) => {
      console.error('Client Service MongoDB connection error:', error);
      this._isConnected = false;
    });

    this.connection.on('disconnected', () => {
      console.log('Client Service MongoDB connection disconnected');
      this._isConnected = false;
    });

    this.connection.on('reconnected', () => {
      console.log('Client Service MongoDB connection reestablished');
      this._isConnected = true;
    });
  }
